...
//  ここで OFF->ON 表示切り替え
  $id=intval($_GET["id"]);
  $sqlCheck=$pdo ->prepare("SELECT * FROM workImage WHERE id= ".$id." and flag=0;");
  $sqlCheck->execute();
  while($row = $sqlCheck -> fetch(PDO::FETCH_ASSOC)){
    if($row){
//      print "OK";
//      ここでONに！
      $DB->workImageOn(intval($row["id"]),"1");
    }else{
//      print "NG";
    }
  }
...
//  ここで ON->OFF 表示切り替え
  ...
  $sqlCheck=$pdo ->prepare("SELECT * FROM workImage WHERE id= ".$id." and flag=1;");
  $sqlCheck->execute();
  while($row = $sqlCheck -> fetch(PDO::FETCH_ASSOC)){
    if($row){
//      print "OK";
//      ここでONに！
      $DB->workImageOn(intval($row["id"]),"0");
    }else{
//      print "NG";
    }
  }
...
if (isset($_POST['del'])) {
  ...
  $sqlCheck=$pdo ->prepare("SELECT * FROM workImage WHERE id= ".$del_id." and flag=0;");
  $sqlCheck->execute();
  while($row = $sqlCheck -> fetch(PDO::FETCH_ASSOC)){
    if($row){
      $DB->workImageDelete(intval($row["id"]));
    }
  }
}
...
<!--サイトマップ-->
    <div class="navi">
     <ul>
      ...
      <li class="now"><a href="./imageWorks.php">作品画像一覧</a></li>
    </ul>
    </div>
    <hr>
...
<?php
  // ON画像があればココに表示
  $sqlImg=$pdo ->prepare("SELECT * FROM workImage WHERE flag=1 ORDER BY id DESC");
...
<?php
  // OFF画像があればココに表示
//  $sqlImg=$pdo ->prepare("SELECT * FROM workImage ORDER BY id DESC LIMIT :ps , :pe ;");
  $sqlImg=$pdo ->prepare("SELECT * FROM workImage WHERE flag=0 ORDER BY id DESC;");
//  $sqlImg->bindValue(':ps', $ps, PDO::PARAM_INT);
//  $sqlImg->bindValue(':pe', PAGING, PDO::PARAM_INT);
  $sqlImg->execute();
  while($rowImg = $sqlImg -> fetch(PDO::FETCH_ASSOC)){
